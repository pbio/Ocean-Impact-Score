...
export async function getServerSideProps() { 
  ...
  const baseUrl:string = API_URL + 'time-series/esg-score/US500?API_KEY=' + API_KEY + '&interval=';
  ...
  const infoUrl:string = API_URL + 'general-info/US500?API_KEY=' + API_KEY + tickers;
  const urls:string[] = [ yearlyUrl,  dailyUrl, infoUrl ];
  // Get json here from Waves of Change (esg cafe) server. 
  const [ yearlyScores,  dailyScores, EU600Info  ] = await Promise.all(urls.map(async (url:string) => {
...
export default function Home({ Info,  yearlyScores, dailyScores}:{ Info:any[], dailyScores:any, yearlyScores: any }) {
  ...
  const organisedScoreData:any = Info.map(company => {
    ...
    companyScores.monthlyScores = [[thisMonthKey, yearlyScores[thisMonthKey][ticker]?.Tone],
                                  [lastMonthKey, yearlyScores[lastMonthKey][ticker]?.Tone]];
    //const monthScore = monthDateKeys.reduce((total, dateIdx) =>{ return total + monthlyScores[dateIdx][company.ticker]?.Tone }, 0) / monthDateKeys.length; //too expensive
